...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
 ...
 //if (r1>0.*km && r2<r1) {
   ...
   if (fabs(fRadius-r2)>1.*km)  {
     /*G4cout<<"r2 "<<istp<<": "<<name<<"  r="<<r1/km<<"-"<<r2/km<<" km  xyz="<<" / "<<xtrack/km<<
       " / "<<ytrack/km<<" / "<<ztrack/km<<"  rXY="<<rXY/km<<"  R-r2="<<(fRadius-r2)/km<<"  R-r1="<<
       (fRadius-r1)/km<<G4endl;*/
     G4AnalysisManager::Instance()->FillH1(10,1);
     if (rXY<100.*km) G4AnalysisManager::Instance()->FillH1(10,2);
   }
   else if (fabs(fRadius-r1)>1.*km)  {
     G4AnalysisManager::Instance()->FillH1(10,5);
     /*G4cout<<"r1 "<<istp<<": "<<name<<"  r="<<r1/km<<"-"<<r2/km<<" km  xyz="<<" / "<<xtrack/km<<
       " / "<<ytrack/km<<" / "<<ztrack/km<<"  rXY="<<rXY/km<<"  R-r2="<<(fRadius-r2)/km<<"  R-r1="<<
       (fRadius-r1)/km<<G4endl;*/
     if (rXY<100.*km) G4AnalysisManager::Instance()->FillH1(10,6);
   }
   ...
   // when you consider histos, choose and:
   // - add cut on theta to collect particles falling within an angle,
   // - add cut on rXY to collect particles within a center spot
   // - cut on energy if needed (nothing about that in the paper...)

   if (name=="mu+" || name=="mu-") {
     G4AnalysisManager::Instance()->FillH1(10,10);
     ...
     //if (energy>10*MeV) { // detectable?
       ...
       //}
     G4AnalysisManager::Instance()->FillH1(16,energy*std::cos(pi - theta));
   }
   else if (name=="proton") {
     G4AnalysisManager::Instance()->FillH1(10,11);
     G4AnalysisManager::Instance()->FillH1(8,energy);
     G4AnalysisManager::Instance()->FillH1(12,energy);
     G4AnalysisManager::Instance()->FillH1(14,theta*180./pi);
     //if (energy>100*MeV) { // detectable?
       G4AnalysisManager::Instance()->FillH2(3,xtrack,ytrack);
       //}
     G4AnalysisManager::Instance()->FillH1(17,energy*std::cos(pi - theta));
   }
   else if (name=="pi+" || name=="pi-") {
     G4AnalysisManager::Instance()->FillH1(10,12);
     G4AnalysisManager::Instance()->FillH1(9,energy);
     G4AnalysisManager::Instance()->FillH1(13,energy);
     G4AnalysisManager::Instance()->FillH1(15,theta*180./pi);
     //if (energy>50*MeV) { // detectable?
       G4AnalysisManager::Instance()->FillH2(4,xtrack,ytrack);
       //}
     G4AnalysisManager::Instance()->FillH1(18,energy*std::cos(pi - theta));
   }
   else if (name=="e+" || name=="e-") G4AnalysisManager::Instance()->FillH1(10,13);
 }
